...
def auth_connect(host, port=None):
    ...
    --------
    nds2.connection
        for details of the connection object
    """
    import nds2
...
def set_parameter(connection, key, value):
    """Execute `nds2.connection.set_parameter` handling `AttributeError`s

    This method just catches an `AttributeError` and displays an `NDSWarning`
    instead.
    """
    try:
...
def get_availability(channels, start, end,
    ...
    result = connection.get_availability(channels)
    out = SegmentListDict()
    for name, result in zip(_get_nds2_names(channels), result):
        out[name] = SegmentList([Segment(s.start_time, s.stop_time) for s in
                                result.simple_list()])
    return out
